...
use Cawl\Vendor\OnlinePayments\Sdk\Domain\PaymentStatusOutput;
...
class OrderUpdater
{
    ...
    private function rebuildPaymentsFromOperations(WlopWcOrder $wlopWcOrder, $response) : void
    {
        ...
        foreach ($operations as $op) {
            ...
            $cancelledCents = (int) ($methodTotals[$method]['cancelled'] ?? 0);
            $methodCapturedCents = (int) ($methodCaptures[$method] ?? 0);
            $displayStatus = (string) $op->getStatus();
            $displayStatusCode = $statusCode;
            if ($tenderAmount > 0 && $cancelledCents >= $tenderAmount) {
                $displayStatus = 'CANCELLED';
                $displayStatusCode = 6;
            } elseif ($tenderAmount > 0 && $refundedCents >= $tenderAmount) {
    ...
    /**
     * Walk operations[] and bucket amounts by CAWL paymentMethod string
    ...
    private function aggregateOperationTotalsByMethod(array $operations) : array
    {
        ...
        foreach ($operations as $op) {
            ...
            if (!isset($totals[$method])) {
                $totals[$method] = ['captured' => 0, 'refunded' => 0, 'pending_refund' => 0, 'cancelled' => 0];
            }
    ...
    private function statusBucket(?int $statusCode) : ?string
    {
        ...
        if (\in_array($statusCode, [6, 62, 75], \true)) {
            return 'cancelled';
        }
        return null;
    }
    ...
    protected function updateOrderDetailsMeta(WlopWcOrder $wlopWcOrder, $paymentResponse = null) : void
    {
        ...
        // Cancelled/rejected payments have no acquired amount; fall back to the order amount.
        $acquired = $paymentOutput->getAcquiredAmount() ?? $paymentOutput->getAmountOfMoney();
