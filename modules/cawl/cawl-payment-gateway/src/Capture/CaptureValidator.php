...
namespace Cawl\Vendor\Worldline\WorldlineForWoocommerce\WorldlinePaymentGateway\Capture;
...
{
    /**
     * Checks whether the order uses a supported CAWL payment method.
     */
    ...
    {
        if ($requestedCents <= 0) {
            throw new \Exception(\__('Capture amount must be greater than zero.', 'cawl-for-woocommerce'));
        }
        if ($requestedCents > $availableCents) {
            throw new \Exception(\__('The amount cannot exceed the available amount of the transaction.', 'cawl-for-woocommerce'));
        }
    }
    /**
