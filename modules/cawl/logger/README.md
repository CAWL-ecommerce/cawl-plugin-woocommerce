# CAWL Logger
[![Continuous Integration](https://github.com/worldline/logger/actions/workflows/ci.yml/badge.svg)](https://github.com/worldline/worldline-woocommerce-logging/actions/workflows/ci.yml)

A module for [worldline/modularity][] providing tools for logging. Heavily based on [worldline/product-woocommerce-logging](https://github.com/worldline/product-woocommerce-logging).
...
```BASH
$ composer require worldline/logger
```
...
Extend the `worldline_logger.log_events` service in your application like this:

```php
//my-plugin.php
'worldline_logger.log_events' => function (array $existingLogEvents, \Psr\Container\ContainerInterface $container): array {
    $logEventsToAdd = [
        [
...
If you want to, you can set default source of logs, so it can be added to each entry if none provided per request. To do so, declare service `worldline_logger.log_events`.
For example:
```php
//PluginCore.php
class PluginCore implements \Worldline\Modularity\Module\ExtendingModule {
    public function extensions() : array{
        return [
            //your core module services here
            'worldline_logger.logging_source' => 'My awesome plugin name'
        ];
    }
}
```
...
```php
            'worldline_logger.object_formatter.map' =>
                static function (ContainerInterface $container): array {
                    return [
                        Throwable::class => $container->get('worldline_logger.object_formatter.map.exception'),
                    ];
                },
```
...
The team at [CAWL][] is engineering the Web since 2006.
...
[worldline/modularity]: https://github.com/worldline/modularity
[wp-events]: https://github.com/worldline/wp-events
[container-interop/service-provider]: https://github.com/container-interop/service-provider
[CAWL]: https://worldline.com
