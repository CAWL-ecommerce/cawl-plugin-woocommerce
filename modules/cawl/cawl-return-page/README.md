...
An object implementing the `\Worldline\ReturnPage\StatusCheckerInterface` interface
that determines the current payment status, returning one of the `\Worldline\ReturnPage\ReturnPageStatus` constants.

### Status updater

`return_page.{paymentMethodID}.status_updater`

An object implementing the `\Worldline\ReturnPage\StatusUpdaterInterface` interface.
...
An object implementing the `\Worldline\ReturnPage\ReturnPageRenderInterface` interface
...
An object implementing the `\Worldline\ReturnPage\StatusActionInterface` interface
...
use Worldline\ReturnPage\StatusActionInterface;
use Worldline\ReturnPage\WcOrderStatusChecker;
...
return static function (): array {
    return [
        ...
        'return_page.cawl-for-woocommerce.action.status.cancelled' => static fn(ContainerInterface $c): StatusActionInterface =>
