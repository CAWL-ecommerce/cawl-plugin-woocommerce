...
function updateLogoPreview(logoUrl: string, isDefault: boolean) {
	...
	const logoInput = document.querySelector(
		'#woocommerce_cawl-for-woocommerce_logo_url'
...
async function fetchLogoPreview() {
	try {
		...
		if (response.success && response.data.logo_url) {
			...
			const logoInput = document.querySelector('#woocommerce_cawl-for-woocommerce_logo_url') as HTMLInputElement | null;
			if (logoInput) {
...
function validateWebhookField(event?: Event) {
	...
	updateGlobalWebhookError(globalErrorNeeded, wp.i18n.__(GLOBAL_WEBHOOK_ERROR_MESSAGE, 'cawl-for-woocommerce'));
...
function getCustomIcons(): CustomIcon[] {
    const customIconsInput = document.querySelector(
        '#woocommerce_cawl-hosted-tokenization_custom_icons'
    ) as HTMLInputElement | null;
...
document.addEventListener('DOMContentLoaded', () => {
	...
	function updateLiveTestFields() {
		...
		setVisibleByClass(
			getFieldRow('#woocommerce_cawl-for-woocommerce_test_api_key'),
			!isLive,
			'wlop-hidden'
		);
		setVisibleByClass(
	...
	function updateAuthorizationFields() {
		...
		setVisibleByClass(
			getFieldRow('#woocommerce_cawl-for-woocommerce_capture_mode'),
			lstAuthorizationMode.value === 'authorization',
			'wlop-hidden'
		);
	}
	...
	function update3dsFields() {
		...
		setVisibleByClass(
			getFieldRow('#woocommerce_cawl-for-woocommerce_enforce_3dsv2'),
	...
	function updateWebhookModeFields() {
		...
		webhookWarningBox.innerHTML = `
    		<p>${wp.i18n.__(WARNING_MESSAGE, 'cawl-for-woocommerce')}</p>
		`;
	...
	setVisibleByClass(
		getFieldRow('#woocommerce_cawl-for-woocommerce_logo_url'),
		false,
		'wlop-hidden'
	);
