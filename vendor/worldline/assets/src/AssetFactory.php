...
use Worldline\Assets\Exception\InvalidArgumentException;
use Worldline\Assets\Loader\PhpFileLoader;
use Worldline\Assets\Loader\ArrayLoader;
/**
 * Class AssetFactory
 *
 * @package Worldline\Assets
 */
final class AssetFactory
{
     ...
     *
     * phpcs:disable Worldline.CodeQuality.FunctionLength.TooLong
     * phpcs:disable Worldline.CodeQuality.NestingLevel.High
     * phpcs:disable Generic.Metrics.CyclomaticComplexity.TooHigh
     * @psalm-suppress MixedArgument, MixedMethodCall
     */
    public static function create(array $config) : \Worldline\Assets\Asset
    {
        $config = self::validateConfig($config);
        $location = $config['location'] ?? \Worldline\Assets\Asset::FRONTEND;
        $handle = $config['handle'];
        $url = $config['url'];
        $class = (string) $config['type'];
        if (!\class_exists($class)) {
            throw new \Worldline\Assets\Exception\InvalidArgumentException(\sprintf('The given class "%s" does not exists.', $class));
        }
        $asset = new $class($handle, $url, $location);
        if (!$asset instanceof \Worldline\Assets\Asset) {
            throw new \Worldline\Assets\Exception\InvalidArgumentException(\sprintf('The given class "%s" is not implementing %s', $class, \Worldline\Assets\Asset::class));
        }
        ...
        if ($asset instanceof \Worldline\Assets\Script) {
        ...
        if ($asset instanceof \Worldline\Assets\Style) {
    ...
    private static function ensureRequiredConfigFields(array $config) : void
    {
        ...
        foreach ($requiredFields as $key) {
            if (!isset($config[$key])) {
                throw new \Worldline\Assets\Exception\MissingArgumentException(\sprintf('The given config <code>%s</code> is missing.', $key));
            }
        }
    }
    ...
    private static function normalizeTranslationConfig(array $config) : array
    {
        ...
        if (!isset($config['translation']['domain'])) {
            throw new \Worldline\Assets\Exception\MissingArgumentException('Config key <code>translation[domain]</code> is missing.');
        }
